# 在hqzcsj增加一个tab页,名为"派出所警情案件统计",代码前缀为"pcsjqajtj_*",后端(py)and前端code(js,html)all放在hqzcsj中对应的子文件夹中
## 数据源:参考hqzcsj\templates\zfba_jq_aj_tab.html
## 分组字段:使用"派出所代码",匹配逻辑是截取"办案单位代码"前8位,然后再后面拼接'0000'
## 只显示(其中括号中的字段通过"显示比例"滑动按钮控制):"警情","同比警情",("同比警情比例"),"转案数","同比转案数","(同比转案数比例)","行政","同比行政",(同比行政比例),"刑事","同比刑事",(同比刑事比例),"行政处罚","同比行政处罚",(同比行政处罚比例),"刑拘","同比刑拘",(同比刑拘比例),"逮捕","同比逮捕",(同比逮捕比例),"起诉","同比起诉",(同比起诉比例)
### 管辖单位映射表stdata.b_dic_zzjgdm的"sspcsdm"与ywdata_zfba_ajxx表的substring(ajxx_cbdw_bh_dm,8)||'0000'值匹配,显示使用sspcs显示
### 筛选区 
#### 1. 时间:
    - 格式:时间区间,包含开始和结束,显示格式为'YYYY-MM-DD HH:MM:SS'
    - 默认值:
        + 开始:当前日期向前推7天,如今天是'2026-01-08 00:00:00',默认为'2026-01-01:00:00:00'
        + 结束:当前日期0点,如今天是'2026-01-08 00:00:00',默认为'2026-01-08:00:00:00'
    - 过滤对应表的对应字段
      +案件
#### 2. 分局:通过ajxx_cbqy_bh_dm过滤,初始化默认不选,即过滤全部,
    - 格式: 多选下拉框
    - 数据源为{云城:445302,云安:445303,罗定:445381,新兴:445321,郁南:445322}
#### 3. 显示比例:
    - 格式:滑动开关
    - 开启后数据集表格显示同比数据比例,百分比2为小数
      - 当同比数值为0时,当前数值不为0时,显示结果为上升X起
      - 当当前数值为0,同比数不为0时,显示结果为下降X起
      - 当同比数值=当前数值时,显示结果为'持平'
### 数据展示区:显示案件编号,案件类型,案件名称,案件状态,案由,ajxx_cbdw_bh as 管辖单位,ajxx_cbqy_bh 分局,发案地点,发案时间,立案时间,简要案情
#### 1. 右上角"导出"按钮,导出的值为"查询"按钮查询到的结果集
    - 逻辑:单击"导出"显示下拉'xlsx','csv'两个选项,点击对应选项下载对应格式文件
    - 文件名:{开始}至{结束}_{分局}+{sspcs}警情案件统计{时间戳}.xlsx/csv
# 1. 在jszahzyj中新增tab页,名为"精神病人警情案件统计",代码前缀为"jsbrjqajtj_*",后端(py)and前端code(js,html)all放在gzrzdd中对应的子文件夹中
### 数据源
    ```
    WITH
    -- Step1: 筛选 2026年以来的精神病相关警情
    -- 策略：spiritcausetrouble 有值 OR 报警内容含关键词 OR 类型名称含精神
    jq_jsb AS (
        SELECT
            caseno                                      AS 警情编号,
            calltime                                    AS 报警时间,
            cmdname AS 地区,
            dutydeptname AS 管辖单位,callerphone AS 报警电话,
            neworicharasubclassname                     AS 原始警情,        
            newcharasubclassname                        AS 确认警情,
            occuraddress                                AS 发生地址,
            casecontents                    AS 报警内容,
            replies                        AS 处警情况,
            spiritcausetrouble                          AS 精神病肇事标识
        FROM ywdata.zq_kshddpt_dsjfx_jq
        WHERE
            -- 时间范围：2026年1月1日起
            calltime >= '2026-01-01' and calltime <'2026-02-01'
            -- 精神病相关识别（任一满足即可）
            -- ~* 为正则不区分大小写匹配，| 等同多个 OR ILIKE，写法更简洁
            AND (
                spiritcausetrouble IS NOT NULL
                OR casecontents         ~* '精神病|精神障碍|精神异常|精神发病|犯病|肇事肇祸'
                OR replies              ~* '精神病|精神障碍|精神异常|精神发病|犯病|肇事肇祸'
                OR charaname            ~* '精神'
                OR newcharasubclassname ~* '精神'
            )
    )

    -- ============================================================
    -- Part A：警情明细 + 是否立案
    -- ============================================================
    SELECT
        jq.警情编号,
        jq.报警时间,jq.地区,jq.管辖单位,jq.报警电话,
        jq.原始警情,
        jq.确认警情,
        jq.发生地址,
        jq.报警内容,
        jq.处警情况,
        jq.精神病肇事标识,
        aj.ajxx_ajbh                                    AS 案件编号,
        aj.ajxx_ajmc                                    AS 案件名称,
        aj.ajxx_ajlx                                    AS 案件类型,
        TO_CHAR(aj.ajxx_lasj, 'YYYY-MM-DD')             AS 立案时间,
        CASE WHEN aj.ajxx_jqbh IS NOT NULL THEN '是' ELSE '否' END AS 是否立案
    FROM jq_jsb jq
    LEFT JOIN ywdata.zq_zfba_ajxx aj
        ON aj.ajxx_jqbh = jq.警情编号
    ORDER BY jq.报警时间 DESC;
    ```
### 筛选区
#### 1. 报警时间
    - 格式:时间区间,包含开始和结束,显示格式为'YYYY-MM-DD HH:MM:SS'
    - 默认值:
        + 开始:当前日期向前推7天,如今天是'2026-01-08 00:00:00',默认为'2026-01-01:00:00:00'
        + 结束:当前日期0点,如今天是'2026-01-08 00:00:00',默认为'2026-01-08:00:00:00'
#### 2. 分局:
    - 格式: 多选下拉框
    - 数据源为stdata.b_dic_zzjgdm表,通过对ssfjdm分组去重,显示用ssfj,实际代码为ssfjdm,过滤使用ssfjdm,对结果集中的"cmdid"过滤
#### 3. "查询"按钮
### 数据展示区:默认为数据源查询到的字段
#### 1. 右上角"导出"按钮,导出的值为"查询"按钮查询到的结果集
    - 逻辑:单击"导出"显示下拉'xlsx','csv'两个选项,点击对应选项下载对应格式文件
    - 文件名:{开始}至{结束}_涉精神病人警情+{时间戳}.xlsx/csv
# 1. 新增tab页,名为"关注人员工作日志"代码前缀为"gzrygzrz_*",后端(py)and前端code(js,html)all放在gzrzdd中对应的子文件夹中
## 数据源
```SQL
select
  *
from
  (
    SELECT
      a.ryxm AS 姓名,
      a.zjhm AS 证件号码,
      CASE WHEN a.sfyjjfx =1 THEN '是' WHEN a.sfyjjfx =0 THEN '否' ELSE a.sfyjjfx END AS 两会是否有进京风险,
      a.lxdh as 联系电话,
      CASE
        WHEN substring(a.sjgsdwdm, 1, 6)='445302' THEN '云城分局'
        WHEN substring(a.sjgsdwdm, 1, 6)='445303' THEN '云安分局'
        WHEN substring(a.sjgsdwdm, 1, 6)='445321' THEN '新兴县公安局'
        WHEN substring(a.sjgsdwdm, 1, 6)='445381' THEN '罗定市公安局'
        WHEN substring(a.sjgsdwdm, 1, 6)='445322' THEN '郁南县公安局'
        ELSE a.sjgsdwdm
      END AS 分局名称,
      d.sspcs AS "所属派出所",
      a.djsj AS "数据登记时间",
      b.djsj AS "工作日志系统登记时间",
      b.gzkzsj:: date AS "工作日志-工作开展时间",
      c.detail AS "工作日志-工作方式",
      b.gzqksm AS "工作日志-工作开展情况",
      case when b.sfczjjzx=1 then '是' when b.sfczjjzx=0 then '否' else b.sfczjjzx end AS 是否存在进京指向,
      b.mqryszwz AS 目前人员所在位置   
    FROM
      stdata.b_per_gzry a
      LEFT JOIN (
        SELECT
          *
        FROM
          stdata.b_per_gzrygzrz
        WHERE
          "deleteflag"=0 and djsj>='2026-02-26' and gzqksm ='测试数据'
      ) b ON a.id=b."flid"
      LEFT JOIN (
        SELECT
          code,
          detail
        FROM
          stdata.s_sg_dict
        WHERE
          "kind_code"='stgzrzfs'
      ) c ON b.stgzfs=c.code
      LEFT JOIN stdata.b_dic_zzjgdm d ON a.sjgsdwdm=d.sspcsdm
    WHERE
      a."deleteflag"=0 
  ) 
```
## 筛选区
### 1. "是否存在进京指向"通过结果集的"sfczjjzx值过滤,默认为空
### 2. "分局":
    - 格式:多选下拉框
    - 数据源:对结果集的"分局名称"字段分组去重结果集
### 3. "工作日志系统登记时间",
    - 格式:时间区间,包含开始和结束,显示格式为'YYYY-MM-DD HH:MM:SS'
    - 默认值:
        + 开始:当前日期向前推7天,如今天是'2026-01-08 00:00:00',默认为'2026-01-01:00:00:00'
        + 结束:当前日期0点,如今天是'2026-01-08 00:00:00',默认为'2026-01-08:00:00:00'
### 4. "查询"按钮
## 数据展示区:默认为数据源查询到的字段
### 1. 右上角"导出"按钮,导出的值为"查询"按钮查询到的结果集
    - 逻辑:单击"导出"显示下拉'xlsx','csv'两个选项,点击对应选项下载对应格式文件
    - 文件名:{开始}至{结束}_关注人员工作日志+{时间戳}.xlsx/csv
# 2. 修改tab页名称
## 1. 修改"工作日志重复度统计"名为"矛盾纠纷风险人员工作日志重复度统计"
## 2. 修改"工作日志超期统计"名为"矛盾纠纷风险人员工作日志超期统计"
# 帮我写一个sql,主要是查询各个时间范围内各派出所的警情,转案,行政,刑事,办结行政,破案,高质量,治拘,刑拘,逮捕,起诉的计数
## 派出所表stdata.b_dic_zzjgdm
  - sspcs 派出所名称
  - sspcsdm 派出所代码
## 警情表ywdata.zq_kshddpt_dsjfx_jq(警情,转案)
  - left(ajxx_cbqy_bh_dm,8)||'0000' 派出所代码
  - calltime 时间
  - 转案(警情表通过`caseno`和案件表的`ajxx_jqbh`字段匹配,匹配到则为"转案")
## 案件表ywdata.zq_zfba_ajxx(行政、刑事)
  - left(ajxx_cbqy_bh_dm,8)||'0000' 派出所代码
  - ajxx_lasj 时间
  - ajxx_ajzt 案件状态,当
    + ajxx_ajlx='行政'且案件状态 NOT IN ('已立案','已受理')则为"办结行政"
    + ajxx_ajlx='刑事'且案件状态 NOT IN ('已立案','已受理')则为"破案"
  - 高质量打击:刑拘大于等于3人以上(案件表通过`ajxx_ajbh`与拘留证表的`ajxx_ajbh`字段匹配,对`ajxx_ajbh`分组计数,如果计数值大于2则属于"高质量")
## 治拘表ywdata.zq_zfba_xzcfjds(治拘),只过滤xzcfjds_cfzl ~ '拘留'的数据
  - left(xzcfjds_cbdw_bh_dm,8)||'0000' 派出所代码
  - xzcfjds_spsj 时间
## 刑拘表ywdata.zq_zfba_jlz
  - left(jlz_cbdw_bh_dm,8)||'0000' 派出所代码
  - jlz_pzsj 时间
## 逮捕表ywdata.zq_zfba_dbz
  - left(dbz_cbqy_bh_dm,8)||'0000' 派出所代码
  - dbz_pzsj 时间
## 起诉表ywdata.zq_zfba_qsryxx
  - left(qsryxx_cbdw_bh_dm,8)||'0000' 派出所代码
  - qsryxx_tfsj 时间
## 当前各表数据量
zq_zfba_dbz,逮捕证,3292,
zq_zfba_jlz,拘留证,7003,
zq_zfba_ajxx,案件信息,29240,
zq_kshddpt_dsjfx_jq,可视化调度平台-大数据分析-警情,1207412,
zq_zfba_qsryxx,起诉人员信息,4454,58:44.9
zq_zfba_xzcfjds,行政处罚决定书,29363,
-- ====================================================
-- 各派出所警情案件综合统计
-- 修改 start_time / end_time 切换统计时间范围
-- ====================================================
WITH
params AS (
    SELECT
        '2026-01-01 00:00:00'::timestamp AS start_time,
        '2026-03-04 00:00:00'::timestamp AS end_time
),

-- 高质量案件：同一 ajxx_ajbh 在拘留证表中出现 ≥3 条
high_quality_cases AS (
    SELECT ajxx_ajbh
    FROM   ywdata.zq_zfba_jlz
    GROUP  BY ajxx_ajbh
    HAVING COUNT(*) >= 3
),

-- 警情 + 转案（警情表用 dutydeptno 作为派出所代码）
jq_stats AS (
    SELECT
        left(jq.dutydeptno, 8) || '0000'       AS pcsdm,
        COUNT(*)                                 AS jq_cnt,
        COUNT(aj.ajxx_jqbh)                      AS za_cnt   -- 匹配到案件则为转案
    FROM   ywdata.zq_kshddpt_dsjfx_jq jq
    CROSS  JOIN params p
    LEFT   JOIN ywdata.zq_zfba_ajxx aj
           ON  jq.caseno = aj.ajxx_jqbh
    WHERE  jq.calltime::timestamp >= p.start_time
      AND  jq.calltime::timestamp <  p.end_time
    GROUP  BY left(jq.dutydeptno, 8) || '0000'
),

-- 行政、刑事、办结行政、破案、高质量
aj_stats AS (
    SELECT
        left(aj.ajxx_cbqy_bh_dm, 8) || '0000'                          AS pcsdm,
        -- 行政案件
        COUNT(*) FILTER (WHERE aj.ajxx_ajlx = '行政')                  AS xz_cnt,
        -- 刑事案件
        COUNT(*) FILTER (WHERE aj.ajxx_ajlx = '刑事')                  AS xs_cnt,
        -- 办结行政：行政 且 案件状态不在 已立案/已受理
        COUNT(*) FILTER (
            WHERE aj.ajxx_ajlx = '行政'
              AND aj.ajxx_ajzt NOT IN ('已立案', '已受理')
        )                                                                AS bjxz_cnt,
        -- 破案：刑事 且 案件状态不在 已立案/已受理
        COUNT(*) FILTER (
            WHERE aj.ajxx_ajlx = '刑事'
              AND aj.ajxx_ajzt NOT IN ('已立案', '已受理')
        )                                                                AS pa_cnt,
        -- 高质量打击：关联拘留证 ≥3 人的案件数
        COUNT(DISTINCT aj.ajxx_ajbh) FILTER (
            WHERE hq.ajxx_ajbh IS NOT NULL
        )                                                                AS gzl_cnt
    FROM   ywdata.zq_zfba_ajxx aj
    CROSS  JOIN params p
    LEFT   JOIN high_quality_cases hq ON hq.ajxx_ajbh = aj.ajxx_ajbh
    WHERE  aj.ajxx_lasj >= p.start_time
      AND  aj.ajxx_lasj <  p.end_time
    GROUP  BY left(aj.ajxx_cbqy_bh_dm, 8) || '0000'
),

-- 治拘（行政拘留）：过滤 cfzl ~ '拘留'
zhiju_stats AS (
    SELECT
        left(xzcfjds_cbdw_bh_dm, 8) || '0000'  AS pcsdm,
        COUNT(*)                                 AS zhiju_cnt
    FROM   ywdata.zq_zfba_xzcfjds
    CROSS  JOIN params p
    WHERE  xzcfjds_cfzl ~ '拘留'
      AND  xzcfjds_spsj >= p.start_time
      AND  xzcfjds_spsj <  p.end_time
    GROUP  BY left(xzcfjds_cbdw_bh_dm, 8) || '0000'
),

-- 刑拘
xingju_stats AS (
    SELECT
        left(jlz_cbdw_bh_dm, 8) || '0000'      AS pcsdm,
        COUNT(*)                                 AS xingju_cnt
    FROM   ywdata.zq_zfba_jlz
    CROSS  JOIN params p
    WHERE  jlz_pzsj >= p.start_time
      AND  jlz_pzsj <  p.end_time
    GROUP  BY left(jlz_cbdw_bh_dm, 8) || '0000'
),

-- 逮捕
daibu_stats AS (
    SELECT
        left(dbz_cbqy_bh_dm, 8) || '0000'      AS pcsdm,
        COUNT(*)                                 AS daibu_cnt
    FROM   ywdata.zq_zfba_dbz
    CROSS  JOIN params p
    WHERE  dbz_pzsj >= p.start_time
      AND  dbz_pzsj <  p.end_time
    GROUP  BY left(dbz_cbqy_bh_dm, 8) || '0000'
),

-- 起诉
qisu_stats AS (
    SELECT
        left(qsryxx_cbdw_bh_dm, 8) || '0000'   AS pcsdm,
        COUNT(*)                                 AS qisu_cnt
    FROM   ywdata.zq_zfba_qsryxx
    CROSS  JOIN params p
    WHERE  qsryxx_tfsj >= p.start_time
      AND  qsryxx_tfsj <  p.end_time
    GROUP  BY left(qsryxx_cbdw_bh_dm, 8) || '0000'
)

SELECT
    d.sspcs                          AS 派出所名称,
    d.sspcsdm                        AS 派出所代码,
    COALESCE(j.jq_cnt,    0)         AS 警情,
    COALESCE(j.za_cnt,    0)         AS 转案,
    COALESCE(a.xz_cnt,    0)         AS 行政,
    COALESCE(a.xs_cnt,    0)         AS 刑事,
    COALESCE(a.bjxz_cnt,  0)         AS 办结行政,
    COALESCE(a.pa_cnt,    0)         AS 破案,
    COALESCE(a.gzl_cnt,   0)         AS 高质量,
    COALESCE(z.zhiju_cnt, 0)         AS 治拘,
    COALESCE(x.xingju_cnt,0)         AS 刑拘,
    COALESCE(db.daibu_cnt,0)         AS 逮捕,
    COALESCE(q.qisu_cnt,  0)         AS 起诉
FROM       stdata.b_dic_zzjgdm d
LEFT JOIN  jq_stats     j  ON j.pcsdm  = d.sspcsdm
LEFT JOIN  aj_stats     a  ON a.pcsdm  = d.sspcsdm
LEFT JOIN  zhiju_stats  z  ON z.pcsdm  = d.sspcsdm
LEFT JOIN  xingju_stats x  ON x.pcsdm  = d.sspcsdm
LEFT JOIN  daibu_stats  db ON db.pcsdm = d.sspcsdm
LEFT JOIN  qisu_stats   q  ON q.pcsdm  = d.sspcsdm
ORDER BY   d.sspcs;