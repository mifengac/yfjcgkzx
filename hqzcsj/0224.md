...
### 数据展示区(参考hqzcsj\templates\zfba_wcnr_jqaj_tab.html的查询区),其中()中的数据项是可根据"显示环比","显示比例"滑动开关'显示/隐藏'的:
    ...
    - 行政、同比行政、刑事、同比刑事(环比行政、环比刑事、同比行政比例、同比刑事比例、环比行政比例、环比刑事比例)
    - 案件(被侵害)、同比案件(被侵害)(环比案件(被侵害)、同比案件(被侵害)比例、环比案件(被侵害)比例)
    - 违法犯罪人员、同比违法犯罪人数(环比违法犯罪人数、同比违法犯罪人数比例、环比违法犯罪人数比例、):数据源使用hqzcsj\templates\wcnr_1393zhibiao_tab.html中的违法犯罪人数
    - 专门教育学生结业后再犯数(专门教育学生结业后再犯率):
        ...
        + 专门教育结业人数:zq_wcnr_sfzxx("jz_time"<6),时间范围'lx_time',类型'jzyy',身份证号'sfzhm
    - 专门(矫治)教育学生结业后再犯数(专门(矫治)教育学生结业后再犯率):
        ...
        + 专门(矫治)教育结业人数:zq_wcnr_sfzxx,时间范围'lx_time',类型'jzyy',身份证号'sfzhm
    ...
    - 刑事占比、同比刑事占比:
        + 显示格式为'X1/X2',X1为未成年人刑事案件数,数据源为hqzcsj\templates\zfba_jq_aj_tab.html中的"刑事",X2为刑事案件数,数据源为hqzcsj\templates\zfba_jq_aj_tab.html中的"刑事"
    ...
    - 严重不良未成年人矫治教育占比
        ...
        + X1为严重不良未成年人矫治人数,数据底数为hqzcsj\templates\jzqk_tongji_tab.html的数据源
            - 过滤条件(符合下方任意条件即满足):
                ...
                + "是否开具矫治文书"值为'是'的
    - 涉刑人员送矫占比:
         + 显示格式为'X1/X2',X1为涉刑人员送矫数,X2涉刑人员数,数据源为hqzcsj\templates\zfba_jq_aj_tab.html中的"刑事"
    - 专门(矫治)教育占比:
    ...
    - 纳管人员再犯占比:
         + 显示格式为'X1/X2',X1为纳管人员再犯数,X2纳管人员数,数据源为hqzcsj\templates\zfba_jq_aj_tab.html中的"刑事"
...
- 字段含义变更：`ywdata.zq_wcnr_sfzxx.jz_time` 由日期口径改为“矫治时长（月）”整数。
