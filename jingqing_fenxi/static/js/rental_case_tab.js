(function() {
    ...
    function getSelectedBranches() {
        var boxes = document.querySelectorAll('#rentalBranchDropdown input[type="checkbox"]');
        var values = [];
    ...
    function renderBranchLabel() {
        var labelNode = document.querySelector('#rentalBranchDisplay span');
        if (!labelNode) return;
    ...
    function renderBranchOptions(branches) {
        ...
        var dropdown = document.getElementById('rentalBranchDropdown');
        if (!dropdown) return;
        dropdown.innerHTML = '';
    ...
    function setError(message) {
        var box = document.getElementById('rentalErr');
        if (!box) return;
        if (message) {
    ...
    function renderPagination() {
        var container = document.getElementById('rentalPagination');
        if (!container) return;
        ...
        container.innerHTML =
            ...
            '<button type="button" id="rentalPrevPage" ' + (state.pageNum <= 1 ? 'disabled' : '') + '>上一页</button>' +
            '<button type="button" id="rentalNextPage" ' + (state.pageNum >= totalPages ? 'disabled' : '') + '>下一页</button>' +
            '</div>';
        document.getElementById('rentalPrevPage').addEventListener('click', function() {
            if (state.pageNum > 1) query(state.pageNum - 1);
        });
        document.getElementById('rentalNextPage').addEventListener('click', function() {
            if (state.pageNum < totalPages) query(state.pageNum + 1);
        });
    }

    function renderStatus(result) {
        var node = document.getElementById('rentalStatus');
        if (!node) return;
    ...
    function buildPayload(pageNum) {
        return {
            start_time: formatRequestTime(document.getElementById('rentalStartTime').value),
            end_time: formatRequestTime(document.getElementById('rentalEndTime').value),
    ...
    function query(pageNum) {
        ...
        fetch('/jingqing_fenxi/api/rental-case/query', {
            method: 'POST',
    ...
    function exportData(format) {
        ...
        window.location.href = '/jingqing_fenxi/download/rental-case?' + params.toString();
    }

    function bindEvents() {
        var display = document.getElementById('rentalBranchDisplay');
        var dropdown = document.getElementById('rentalBranchDropdown');
        var menu = document.querySelector('#rentalDownloadMenu .download-menu-content');
        if (display) {
        ...
        document.querySelector('[data-special-case-type="rental"] [data-action="query"]').addEventListener('click', function() {
            query(1);
        });
        document.querySelector('#rentalDownloadMenu [data-action="toggle-export"]').addEventListener('click', function(event) {
            event.stopPropagation();
            menu.classList.toggle('show');
        });
        document.querySelectorAll('#rentalDownloadMenu [data-action="export"]').forEach(function(button) {
    ...
    function initDefaults() {
        fetch('/jingqing_fenxi/api/rental-case/defaults')
            .then(function(response) { return response.json(); })
            .then(function(result) {
                if (!result.success) throw new Error(result.message || '初始化失败');
                document.getElementById('rentalStartTime').value = formatDateTimeLocal(result.start_time);
                document.getElementById('rentalEndTime').value = formatDateTimeLocal(result.end_time);
    ...
    function init() {
        if (state.initialized || !document.getElementById('rentalStartTime')) return;
        state.initialized = true;
        bindEvents();
        initDefaults();
    }

    window.RentalCaseTabPage = { init: init };
})();